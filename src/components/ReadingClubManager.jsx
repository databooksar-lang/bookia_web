  ...
  return <div className="reading-club-manager">
    ...
    {!clubsLoading && clubs.length > 0 ? <div className="dashboard-list reading-club-list">{clubs.map((club) => {
      ...
      return <article key={club.id} className="dashboard-card reading-club-item">
        ...
        <div className="card-actions"><div className="card-actions-main">{isEditing ? <button type="button" className="secondary-button" onClick={cancelEditing} disabled={isSaving}>Cancelar</button> : <><button type="button" className="secondary-button" onClick={() => startEditing(club)} disabled={isSaving}>Editar</button><label className="secondary-button">{coverBusyId === club.id ? "Subiendo..." : "Subir portada"}<input className="visually-hidden" type="file" accept="image/png,image/jpeg,image/webp" disabled={coverBusyId !== null} onChange={(event) => { uploadCover(club, event.target.files?.[0]); event.target.value = ""; }} /><small>Medida recomendada: 900 × 1200 px (formato vertical).</small></label>{club.cover_url ? <button type="button" className="secondary-button" onClick={() => removeCover(club)} disabled={coverBusyId !== null}>Quitar portada</button> : null}<button type="button" className="secondary-button" onClick={() => toggleInterests(club.id)} disabled={interestsLoadingId === club.id}>{interestsLoadingId === club.id ? "Cargando..." : interestsByClub[club.id] ? "Ocultar personas interesadas" : "Ver personas interesadas"}</button>{club.is_visible ? <ReadingClubShareMenu club={club} host={host} hostName={hostName} bookstoreId={host?.type === "bookstore" ? host.id : null} source={source} /> : null}</>}{isEditing ? <button type="button" className="primary-button" onClick={() => saveClub(club.id)} disabled={isSaving}>{isSaving ? "Guardando..." : "Guardar"}</button> : null}</div></div>
