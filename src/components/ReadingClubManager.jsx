...
export function ReadingClubCoverUpload({ busy, onUpload }) {
  return <>
    <label className="secondary-button">{busy ? "Subiendo..." : "Subir portada"}<input className="visually-hidden" type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={(event) => { onUpload(event.target.files?.[0]); event.target.value = ""; }} /></label>
    <small className="reading-club-cover-guidance">Medida recomendada: 900 × 1200 px (formato vertical).</small>
  </>;
}
  ...
  return <div className="reading-club-manager">
    <form className={formClassName} onSubmit={createClub}>
      ...
      <div className="dashboard-form-grid dashboard-form-grid-extended">
        ...
        <label className="dashboard-field-wide">Descripción *<RichDescriptionEditor value={newClub.description} onChange={(description) => setNewClub((current) => ({ ...current, description }))} placeholder="Contá de qué trata el encuentro." maxLength={null} required /></label>
    ...
    {!clubsLoading && clubs.length > 0 ? <div className="dashboard-list reading-club-list">{clubs.map((club) => {
      ...
      return <article key={club.id} className="dashboard-card reading-club-item">
        {isEditing ? <div className="dashboard-form-grid dashboard-form-grid-extended">
          ...
          <label className="dashboard-field-wide">Descripción *<RichDescriptionEditor value={draftClub.description} onChange={(description) => setDraftClub((current) => ({ ...current, description }))} placeholder="Contá de qué trata el encuentro." maxLength={null} required /></label>
