...
import { buildBookShareMessage, buildBookShareUrl, buildInstagramStoryCoverPath, buildTelegramShareHref, buildWhatsAppShareHref, copyBookShareUrl, createInstagramStoryFile, shareBookToInstagram, shareInstagramStoryFile } from "../bookSharingState";
...
export function BookShareMenu({ item, bookstore }) {
  ...
  return <div className="book-share-menu">
    ...
    {isOpen ? <div id={menuId} className="book-share-options" role="group" aria-label={`Compartir ${item.title}`}>
      ...
      <button type="button" className="book-share-story-button" onClick={shareInstagramStory} disabled={isStoryBusy}><InstagramIcon size={19} /><BookIcon size={17} /><span>{isStoryBusy ? "Creando Story..." : "Historia de Instagram"}</span></button>
