  ...
  return (
    <section className="dashboard-shell">
        ...
        {!loading && activeItems.length > 0 ? <div className="dashboard-list dashboard-list-active">{activeItems.map((item) => {
          ...
          return (
            <article key={item.id} className={`dashboard-card catalog-item${isEditing ? " is-editing" : ""}`}>
              ...
              <div className="card-actions"><div className="card-actions-main">{isEditing ? <button type="button" className="secondary-button" onClick={cancelEditing} disabled={catalogActionsBusy}>Cancelar</button> : <><button type="button" className="secondary-button catalog-compact-action" onClick={() => startEditing(item)} disabled={catalogActionsBusy}>Editar</button><button type="button" className="secondary-button catalog-compact-action" onClick={() => toggleFeatured(item)} disabled={catalogActionsBusy}>{item.is_featured ? "Quitar destacado" : "Destacar"}</button></>}{canAutocompleteWithAi ? <button type="button" className="secondary-button catalog-ai-autocomplete-button" onClick={() => autocompleteItem(item)} disabled={catalogActionsBusy}>{isAiBusy ? <><SparkleIcon size={16} /> Autocompletando...</> : <><SparkleIcon size={16} /> Autocompletar con IA</>}</button> : null}{isEditing ? <button type="button" className="primary-button catalog-save-button" onClick={() => saveItem(item)} disabled={catalogActionsBusy}>{saveUiState.isCurrentItemSaving ? "Guardando..." : "Guardar"}</button> : null}</div><div className="catalog-secondary-actions">{isEditing ? null : <BookShareMenu item={item} bookstore={me.bookstore} />}<button type="button" className="danger-button catalog-hide-trigger" onClick={() => setPendingHideItem(item)} disabled={catalogActionsBusy}>Eliminar</button></div></div>
            </article>
          );
        })}</div> : null}
      </DashboardPanel>
      ...
      <DashboardPanel
        ...
        description="Administrá importaciones externas desde un solo lugar."
