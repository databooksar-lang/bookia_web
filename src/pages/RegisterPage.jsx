...
export function RegisterPage({ onRegister, onAuthenticated, pendingAction, me, locationSearch }) {
  ...
  if (me) return <Redirect to={getAccountDestination(me)} />;
        ...
        <div className="register-form-panel">
          ...
          <form className="register-form" onSubmit={submit}>
            ...
            {(isReader || bookstoreStep === "account") ? <><label>Correo electrónico<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required /></label><div className="register-password-group">
              ...
              {isTrial ? <p className="billing-notice">Tu prueba incluye hasta 10 libros durante 15 dias, sin costo ni renovación automática.</p> : <fieldset className="register-catalog-options"><legend>Queres ampliar tu catalogo?</legend>
              ...
              <div><span>Plan {planCode === "initial" ? "Manual" : "Plus AI"}</span><strong>{formatCommercialPrice(pricingState.prices?.[planCode] || 0)}/mes</strong></div>
