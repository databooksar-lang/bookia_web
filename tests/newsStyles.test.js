...
export function registerNewsStylesTests(test) {
test("styles public and dashboard news cards responsively", () => {
  ...
  assert.match(styles, /\.news-item-summary\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+max-content;/s);
  assert.match(styles, /\.news-item-summary\.has-image\s*\{[^}]*grid-template-columns:\s*120px\s+minmax\(0,\s*1fr\)\s+max-content;/s);
