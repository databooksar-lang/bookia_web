...
import { registerReadingClubDescriptionRenderTests } from "./readingClubDescriptionRender.test.js";
...
tests.push(["publishes terms and conditions for Bookia's marketplace role", () => {
  ...
  assert.match(termsSource, /Perfiles de autor/);
...
tests.push(["keeps public reading-club cards compact with optional covers and bounded descriptions", () => {
  ...
  assert.match(editorialStyles, /\.reading-club-public-card-content:has\(\.reading-club-public-cover:not\(\[hidden\]\)\)\s*\{[^}]*grid-template-columns:\s*88px\s+minmax\(0,\s*1fr\);/s);
  ...
  assert.match(editorialStyles, /\.reading-club-public-description\s*\{[^}]*line-height:\s*1\.5;[^}]*max-height:\s*6em;[^}]*overflow:\s*hidden;/s);
