...
export function registerAuthorBooksStateTests(test) {
  test("normalizes complete author books and rejects malformed API entries", () => {
    assert.deepEqual(
      normalizeAuthorBooks({
        items: [
          { id: 7, title: " La casa ", synopsis: " Memoria ", genre: { id: 3, name: "Novela", slug: "novela" }, publisher: null, publication_year: 2026, is_hidden: true, cover_url: "/dashboard/author-books/7/cover" },
      ...
      [{ id: 7, title: "La casa", synopsis: "Memoria", genre: { id: 3, name: "Novela", slug: "novela" }, publisher: "", publication_year: 2026, is_hidden: true, cover_url: "/dashboard/author-books/7/cover" }],
    );
  });
  ...
  test("builds create drafts, multipart forms, and trimmed metadata updates", () => {
    ...
    assert.deepEqual(
      ...
      { id: 7, title: "La casa", synopsis: "Memoria", genre_id: "3", publisher: "", publication_year: "2026", is_hidden: true, cover: null },
    );

    const form = buildAuthorBookFormData({ title: "  La casa ", synopsis: " Memoria ", genre_id: "3", publisher: " Sur ", publication_year: "2026", cover });
    ...
    const minimalForm = buildAuthorBookFormData({ title: "Libro", synopsis: "Texto", genre_id: "3", publisher: " ", publication_year: "", cover });
    assert.equal(minimalForm.has("publisher"), false);
    assert.equal(minimalForm.has("publication_year"), false);
    assert.deepEqual(
      buildAuthorBookUpdatePayload({ title: " La casa ", synopsis: " Memoria ", genre_id: "3", publisher: " ", publication_year: "", is_hidden: true }),
      { title: "La casa", synopsis: "Memoria", genre_id: 3, publisher: null, publication_year: null, is_hidden: true },
    );
  });
  ...
  test("uses multipart create and cover replacement while metadata stays JSON", async () => {
    ...
    const cover = new Blob(["cover"], { type: "image/png" });
    const draft = { id: 9, title: "Libro", synopsis: "Texto", genre_id: "3", publisher: "", publication_year: "", is_hidden: false, cover };
    ...
    assert.deepEqual(calls.slice(1), [
      { path: "/dashboard/author-books/9", options: { method: "PATCH", body: JSON.stringify({ title: "Libro", synopsis: "Texto", genre_id: 3, publisher: null, publication_year: null, is_hidden: false }) } },
